...
> **适用**:项目有 chaos 配置或 `/wyyp` 参数里显式要求。验证依赖故障 / 网络扰动下的韧性。
>
> **默认只告警,不阻塞**(`.wyyp.yml` 可改为 block),因为混沌结果波动大。

## 三态判定

| 状态 | 信号 | 打分 |
|------|------|------|
| **applicable** | 有 `chaos-mesh/` / `litmus/` / `toxiproxy.json` / `.chaos/` 或 `.wyyp.yml` 的 chaos 段非空 或 用户显式要求 | 跑(先询问) |
| **MISSING** | 多服务分布式架构 + ≥3 个外部依赖 + 已进入生产运行(有 tag + deployment 配置),但无任何 chaos 配置 | **扣满 5 分** |
| **N/A** | 单体服务 / 单依赖 / MVP / 无生产部署 | 不扣分 |

混沌权重只有 5,MISSING 影响小(扣 5 分,最多从 A+ 掉到 A)。这是故意设计——混沌是"高阶韧性"要求,不是入门门槛。

**MISSING 报告模板**:
```
[混沌] MISSING — 扣 5 分
   检测到 5 个外部依赖(MySQL / Redis / Kafka / S3 / 外部 HTTP API)+ 已发 v2.x tag。
   这个规模应该有基础的 chaos 演练(至少 DB 延迟 / Redis 断连两个场景)。
   建议:引入 toxiproxy,先跑两个简单场景看服务降级行为。
   参考:spec/06-chaos.md#推荐的混沌场景清单
```
